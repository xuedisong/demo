...
public class UserRightInterceptor {
    ...
    @Pointcut("execution(* com.cloudhua.imageplatform.controller.UserController.*(..)) " +
            "&& @annotation(org.springframework.web.bind.annotation.RequestMapping)")
    private void checkUserRightPointcut() {
    }
    @Around("checkUserRightPointcut()")
    public void checkRight(ProceedingJoinPoint pjp) throws Throwable {
        ...
        for (Object obj : args) {
            if (obj instanceof HttpEnv) {//把连接点的HttpEvent参数信息拿出来,在里面寻找right信息
                httpEnv = (HttpEnv) obj;
                break;
            }
        }
        ...
        if (token != null) {
            RightUtil.chechRight(t.getUid(),RightConfig.USER_MANAGE);
            try {
                pjp.proceed(args);
